    ...
    describe('Users', () => {
        ...
        describe('GET /users/:username/routines', () => {
            it('Gets a list of public routines for a particular user.', async () => {
                ...
                const {data: routines} = await axios.get(`${API_URL}/api/users/${
                    userWithRoutines.username
                }/routines`);
    ...
    describe('Activities', () => {
        ...
        describe('PATCH /activities/:activityId (*)', () => {
            it('Anyone can update an activity (yes, this could lead to long term problems a la wikipedia)', async () => {
                ...
                const {data: respondedActivity} = await axios.patch(`${API_URL}/api/activities/${
                    activityToCreateAndUpdate.id
                }`, newActivityData, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
        ...
        describe('GET /activities/:activityId/routines', () => {
            it('Get a list of all public routines which feature that activity', async () => {
                ...
                const {data: routines} = await axios.get(`${API_URL}/api/activities/${
                    testActivity.id
                }/routines`);
    ...
    describe('Routines', () => {
        ...
        describe('POST /routines (*)', () => {
            it('Creates a new routine, with the creatorId matching the logged in user', async () => {
                const {data: respondedRoutine} = await axios.post(`${API_URL}/api/routines`, routineToCreateAndUpdate, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
        ...
        describe('PATCH /routines/:routineId (**)', () => {
            it('Updates a routine, notably changing public/private, the name, or the goal', async () => {
                const {data: respondedRoutine} = await axios.patch(`${API_URL}/api/routines/${
        ...
        describe('DELETE /routines/:routineId (**)', () => {
            it('Hard deletes a routine. Makes sure to delete all the routineActivities whose routine is the one being deleted.', async () => {
                const {data: deletedRoutine} = await axios.delete(`${API_URL}/api/routines/${
                    routineToCreateAndUpdate.id
                }`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
        ...
        describe('POST /routines/:routineId/activities', () => {
            ...
            it('Attaches a single activity to a routine.', async () => {
                ...
                const {data: respondedRoutineActivity} = await axios.post(`${API_URL}/api/routines/${
                    newRoutine.id
                }/activities`, {
            ...
            it('Prevents duplication on (routineId, activityId) pair.', async () => {
                let duplicateIds,
                    duplicateIdsResp;
                try {
                    duplicateIds = await axios.post(`${API_URL}/api/routines/${
                        newRoutine.id
                    }/activities`, routineActivityToCreateAndUpdate, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
    ...
    describe('routine_activities', () => {
        ...
        describe('PATCH /routine_activities/:routineActivityId (**)', () => {
            it('Updates the count or duration on the routine activity', async () => {
                const {data: respondedRoutineActivity} = await axios.patch(`${API_URL}/api/routine_activities/${
            ...
            it('Logged in user should be the owner of the modified object.', async () => {
                let respondedRoutineActivity,
                    errRespondedRoutineActivity;
                try {
                    respondedRoutineActivity = await axios.patch(`${API_URL}/api/routine_activities/${4}`, newRoutineActivityData, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
        ...
        describe('DELETE /routine_activities/:routineActivityId (**)', () => {
            it('Removes an activity from a routine, uses hard delete', async () => {
                const {data: deletedRoutineActivity} = await axios.delete(`${API_URL}/api/routine_activities/${
                    routineActivityToCreateAndUpdate.id
                }`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
            ...
            it('Logged in user should be the owner of the modified object.', async () => {
                let respondedRoutineActivity,
                    errRespondedRoutineActivity;
                try {
                    respondedRoutineActivity = await axios.delete(`${API_URL}/api/routine_activities/${4}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
